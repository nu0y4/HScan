...
def find_by_url(url, js=False):
    if js == False:
        try:
            main.print_green("url:" + url)
        except:
            main.print_red("操你妈输入这样格式的url：https://www.baidu.com")
        html_raw = Extract_html(url)
        if html_raw == None:
            main.print_red("连接失败" + url)
            return None
        # print(html_raw)
...
def find_by_url_deep(url):
    ...
    main.print_red("找到" + str(len(links)) + "条超链接")
    urls = []
    i = len(links)
    for link in links:
        temp_urls = find_by_url(link)
        if temp_urls == None: continue
        main.print_red("Remaining " + str(i) + " | Find " + str(len(temp_urls)) + " URL in " + link)
        for temp_url in temp_urls:
...
def find_by_file(file_path, js=False):
    ...
    for link in links:
        ...
        if temp_urls == None: continue
        main.print_red(str(i) + " Find " + str(len(temp_urls)) + " URL in " + link)
        for temp_url in temp_urls:
            if temp_url not in urls:
                urls.append(temp_url)
        i -= 1
